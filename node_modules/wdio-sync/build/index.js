...
var applyPrototype = function applyPrototype(result, helperScope) {
     ...
     */
    if (!result || (typeof result === 'undefined' ? 'undefined' : (0, _typeof3.default)(result)) !== 'object' || Array.isArray(result) && !_get__('isElementsResult')(result) && !_get__('is$$Result')(result) || Buffer.isBuffer(result)) {
        return result;
    }
     ...
     */
    if (_get__('isElementsResult')(result)) {
    ...
    return origFn(function () {
        ...
        return _get__('executeHooksWithArgs')(before).catch(hookError('beforeHook')).then(function () {
            ...
            if (_get__('isAsync')() || hookFn.name === 'async') {
                return _get__('executeAsync').call(_this6, hookFn, repeatTest);
            }

            return new _promise2.default(_get__('runSync').call(_this6, hookFn, repeatTest));
        }).then(function () {
...
var runSpec = function runSpec(specTitle, specFn, origFn) {
    ...
    if (_get__('isAsync')() || specFn.name === 'async') {
        return origFn(specTitle, function async() {
            return _get__('executeAsync').call(this, specFn, repeatTest);
        });
    }

    return origFn(specTitle, function () {
        return new _promise2.default(_get__('runSync').call(this, specFn, repeatTest));
    });
};
...
function runSync(fn) {
    ...
    return function (resolve, reject) {
        return _get__('Fiber')(function () {
            return _get__('executeSync').call(_this7, fn, repeatTest).then(function () {
                return resolve();
            }, reject);
        }).run();
    };
...
var runInFiberContext = function runInFiberContext(testInterfaceFnNames, before, after, fnName) {
    ...
    global[fnName] = _get__('wrapTestFunction')(fnName, origFn, testInterfaceFnNames, before, after);
    ...
    if (typeof origFn.only === 'function') {
        ...
        global[fnName].only = _get__('wrapTestFunction')(fnName + '.only', origOnlyFn, testInterfaceFnNames, before, after);
    }
};
